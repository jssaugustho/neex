...
const VerifySessionEmail = (props: {
  token: string;
  user: iUser;
  ...
  return (
    <Html>
      <Head />
      <Preview>
        Um novo dispositivo está tentando fazer login na sua conta em{" "}
        {props.session.ip.city}/{props.session.ip.region} às{" "}
      ...
      <Body style={main}>
        <Container style={container}>
          <Section style={box}>
            ...
            <Text style={paragraph}>
              Caso não tenha sido você quem solicitou o envio desse email é
              importante que não clique no link para autorizar. Se desconfiar
              que alguém esteja tentando entrar na sua conta nos notifique
              sobre.
            </Text>
            <Button
              style={button}
              href={`http://${process.env.WEB_CLIENT_ADDRESS}${":" + String(process.env.WEB_CLIENT_PORT)}/login/other/verify-session/${props.token}/verify`}
            >
              Autorizar e Fazer login
            </Button>
            <Hr style={hr} />
            <Text style={paragraph}>
...
export default VerifySessionEmail;
